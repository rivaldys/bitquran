...
/**
 * Recursively collect all routes that define `meta.navbarIcon`.
 *
 * @param routes The array of Route objects to scan.
 * @returns An array of routes that include `meta.navbarIcon`.
 */
const getNavbarRoutes = (routes: Route[]): Route[] =>
{
    ...
    {
        // If the route has a navbar icon, include it
        if(route.meta?.navbarIcon)
        {
            result.push(route)
        }
