...
function NavButton({ onClick, disabled, direction }: { onClick: () => void; disabled: boolean; direction: 'prev' | 'next' }) {
    ...
    return (
        ...
        >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5" viewBox="0 0 512 512" stroke="currentColor">
                {isPrev
                    ? <path fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="48" d="M328 112L184 256l144 144" />
                    : <path fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="48" d="M184 112l144 144-144 144" />
                }
            </svg>
        </button>
    )
...
export default function Tafsir() {
    ...
    return (
        <>
            <Head>
                <title>Tafsir {surahName} Ayat {verse.number.inSurah} — Bitquran</title>
            </Head>

            <div className="bg-white rounded-[7px] sm:rounded-[10px] shadow-[0_0_10px_rgba(234,234,234,0.8)] p-[15px] sm:p-[20px]">
                {/* Navigation Header */}
                <div className="flex items-center justify-between mb-[15px]">
                    <NavButton onClick={() => goTo(currentAyah - 1)} disabled={!hasPrev} direction="prev" />
                    ...
                    <div className="text-center">
                        ...
                        <span className="text-[#999999] text-sm leading-[21px]">Ayat {verse.number.inSurah}</span>
                    </div>

                    <NavButton onClick={() => goTo(currentAyah + 1)} disabled={!hasNext} direction="next" />
                </div>
