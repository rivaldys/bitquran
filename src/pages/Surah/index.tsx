...
function BismillahPlayer({ text, src }: { text: string; src: string }) {
    ...
    return (
        <div className="mt-3.75 flex flex-col items-center gap-2.5">
            ...
            >
                {isPlaying ? (
                    <svg className="h-3.5 sm:h-4 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                    ...
                    <div key={verse.number.inSurah} id={`${surah.number}-${verse.number.inSurah}`}>
                        <VerseItem
                            toolbar={{
                                ...
                                onTafsir: () => navigate(`/surat/${surah.number}/tafsir/${verse.number.inSurah}`)
                            }}
